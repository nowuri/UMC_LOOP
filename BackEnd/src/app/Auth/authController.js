...
(req, res) => {
  const {accessToken} = req.body;
	var naverProfile;
	try{
    naverProfile = axios.get('https://openapi.naver.com/v1/nid/me', { //axios.get 앞에 awiat 뺐는데 그래도 되는지 확인 필요
      headers: {
    ...
    const createNaverUserResult = userService.createNaverUser(newUserData);

    // console.log(createUserResult);
    if (createNaverUserResult.code === 1000) {
      return res.send(createNaverUserResult);
    }
    else if (createNaverUserResult.code === 3003) {
      return res.status(300).send(createNaverUserResult);
    }

    res.status(400).send(createNaverUserResult);
